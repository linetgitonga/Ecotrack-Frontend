...
| **1. Foundation** | P1–P3 | pubspec; `app/themes/*`; `core/error`, `core/constants`, `core/utils` (+ tests); `injection/`; `go_router` skeleton with auth redirect; `AppDatabase` + all tables + migration test; secure storage / prefs. | analyze clean, DB schema test green |
| **2. Core infra** | P4–P6 | Dio factories + full interceptor stack (+ tests); `Result`/`Failure` end-to-end; `ConnectionManager` + mDNS + state machine (+ tests); `SyncEngine` + outbox processor + `workmanager`; `RealtimeClient`; repository base + `sync_meta` staleness. | ConnectivityBloc transitions tested |
