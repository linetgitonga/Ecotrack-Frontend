...
| **3–9 ALL ✅** | — | **Phases 0–9 complete.** Auth (OTP/sessions/step-up), Tenancy (sites/rooms/members, cache-first), Home dashboard, Devices+detail+pairing, Insights (fl_chart), Automation, Alerts, Account, Privacy/DPA-2019 (consents + export/erasure + legal docs), multi-site **Portfolio** (overview/units/billing/maintenance + unit detail), **web dashboard** (drift-WASM DB, cookie notice, responsive `DataTable`/nav-rail, CI web-build smoke). Tier-B screens run on `MockInterceptor` (swap = `mockMode=false`). ~111 tests, analyze clean; android(needs SDK)/ios(CI)/web build. **Deferred (non-blocking):** LAN-to-hub wiring + cert-pin storage (needs hardware), rule/schedule builder editors (stubbed), telemetry rollup caching, Super-Admin area (separate auth path). |
